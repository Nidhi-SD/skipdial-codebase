...
/* Real brand glyphs (CC0-licensed, Simple Icons) — only bundled for brands whose
   mark is freely redistributable. Everything else falls back to a colored
   initial badge rather than an inexact or unlicensed logo. */
...
export function BrandLogo({ name, className }: { name: string; className?: string }) {
  ...
  if (!color) {
    ...
    return <span className={cn("text-[13.5px] font-semibold text-ink-faint", className)}>{name}</span>;
  }
  ...
  return (
    <span className={cn("inline-flex items-center gap-2", className)}>
      ...
      >
        {path ? (
          <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" style={{ fill: color }} aria-hidden>
            <path d={path} />
          </svg>
        ) : (
      ...
      <span className="text-[14.5px] font-bold tracking-tight" style={{ color }}>
        {name}
      </span>
    </span>
  );
